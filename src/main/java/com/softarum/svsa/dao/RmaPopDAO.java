...
public class RmaPopDAO implements Serializable {
	...
	public List<PessoaTO> buscarE1(Unidade unidade, Date ini, Date fim, Sexo sexo, Long tenantId) {
		log.debug("rma pop e1");
		
		/*
			SELECT *  FROM svsaweb.pessoaAbordagem pa
			INNER JOIN svsaweb.abordagem a ON a.codigo_pessoa = pa.codigo                
			WHERE a.codigo_unidade = 10           
				and a.data between '2022-08-01' and '2022-08-31';		
		*/
		
		List<PessoaTO> tos = new ArrayList<PessoaTO>();		
		
		tos = manager.createQuery("SELECT distinct new com.softarum.svsa.modelo.to.rma.PessoaTO(pa.dataNascimento) "
			+ "FROM PessoaAbordagem pa "
			+ "	INNER JOIN Abordagem a ON a.pessoa.codigo = pa.codigo "
			+ "WHERE pa.unidade = :unidade "
			+ " and pa.sexo = :sexo "				
			+ " and pa.tenant_id = :tenantId "
			+ " and a.data between :ini and :fim ", PessoaTO.class)			
		.setParameter("unidade", unidade)
		.setParameter("tenantId", tenantId)
		.setParameter("sexo", sexo)
		.setParameter("ini", ini, TemporalType.TIMESTAMP)
	...
	public Long buscarE2(Unidade unidade, Date ini, Date fim, Long tenantId) {
		...
		/*
			SELECT count(*)  FROM svsaweb.pessoaAbordagem pa
			INNER JOIN svsaweb.abordagem a ON a.codigo_pessoa = pa.codigo                
			WHERE pa.codigo_unidade = 10  
	            and TIMESTAMPDIFF (YEAR, pa.dataNascimento, CURDATE()) between 0 and 100
	            and a.situacao = "TRABALHO_INFANTIL"
				and a.data between '2022-08-01' and '2022-08-31';	
		 */
		
		return manager.createQuery("select count(pa) from PessoaAbordagem pa "
				+ "	INNER JOIN Abordagem a ON a.pessoa.codigo = pa.codigo "
				+ "where pa.unidade = :unidade "
				+ " and TIMESTAMPDIFF (YEAR, pa.dataNascimento, CURDATE()) between 0 and 15"				
				+ " and pa.tenant_id = :tenantId "
				+ " and a.situacao = :situacao "
				+ " and a.data between :ini and :fim ", Long.class)
				.setParameter("unidade", unidade)
				.setParameter("tenantId", tenantId)
				.setParameter("ini", ini, TemporalType.TIMESTAMP)
				.setParameter("fim", DateUtils.plusDay(fim), TemporalType.TIMESTAMP)
				.setParameter("situacao", CodigoAuxiliarAtendimento.TRABALHO_INFANTIL)
				.getSingleResult();
	...
	public Long buscarE3(Unidade unidade, Date ini, Date fim, Long tenantId) {
		...
		return manager.createQuery("select count(pa) from PessoaAbordagem pa "
				+ "	INNER JOIN Abordagem a ON a.pessoa.codigo = pa.codigo "
				+ "where pa.unidade = :unidade "
				+ " and TIMESTAMPDIFF (YEAR, pa.dataNascimento, CURDATE()) between 0 and 17"				
				+ " and pa.tenant_id = :tenantId "
				+ " and a.situacao = :situacao "
				+ " and a.data between :ini and :fim ", Long.class)
				.setParameter("unidade", unidade)
				...
				.setParameter("ini", ini, TemporalType.TIMESTAMP)
				.setParameter("fim", DateUtils.plusDay(fim), TemporalType.TIMESTAMP)
				.setParameter("situacao", CodigoAuxiliarAtendimento.EXPLORACAO_SEXUAL)
				.getSingleResult();
	...
	public Long buscarE4(Unidade unidade, Date ini, Date fim, Long tenantId) {
		...
		return manager.createQuery("select count(pa) from PessoaAbordagem pa "
				...
				.setParameter("ini", ini, TemporalType.TIMESTAMP)
				.setParameter("fim", DateUtils.plusDay(fim), TemporalType.TIMESTAMP)
				.setParameter("situacao", CodigoAuxiliarAtendimento.USUARIO_CRACK_OU_DROGRAS_ILICITAS)
				.getSingleResult();
	...
	public Long buscarE5(Unidade unidade, Date ini, Date fim, Long tenantId) {
		...
		return manager.createQuery("select count(pa) from PessoaAbordagem pa "
				+ "	INNER JOIN Abordagem a ON a.pessoa.codigo = pa.codigo "
				+ "where pa.unidade = :unidade "
				+ " and TIMESTAMPDIFF (YEAR, pa.dataNascimento, CURDATE()) between 18 and 100"				
				+ " and pa.tenant_id = :tenantId "
				+ " and a.situacao = :situacao "
				+ " and a.data between :ini and :fim ", Long.class)
				.setParameter("unidade", unidade)
	...
	public Long buscarE6(Unidade unidade, Date ini, Date fim, Long tenantId) {
		...
		return manager.createQuery("select count(pa) from PessoaAbordagem pa "
				...
				.setParameter("ini", ini, TemporalType.TIMESTAMP)
				.setParameter("fim", DateUtils.plusDay(fim), TemporalType.TIMESTAMP)
				.setParameter("situacao", CodigoAuxiliarAtendimento.MIGRANTE)
				.getSingleResult();
	}
	...
	public Long buscarE7(Unidade unidade, Date ini, Date fim, Long tenantId) {
		...
		return manager.createQuery("select count(pa) from PessoaAbordagem pa "
				...
				.setParameter("ini", ini, TemporalType.TIMESTAMP)
				.setParameter("fim", DateUtils.plusDay(fim), TemporalType.TIMESTAMP)
				.setParameter("situacao", CodigoAuxiliarAtendimento.USUARIO_ALCOOL)
				.getSingleResult();
	}
